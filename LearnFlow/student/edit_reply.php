...
if ($replyId <= 0) { header("Location: discussions.php"); exit(); }
...
$stmt = $conn->prepare("
    SELECT
        dr.reply_id, dr.post_id, dr.content,
        dp.title AS post_title, dp.forum_id,
...
$stmt->close();
if (!$reply) { header("Location: discussions.php"); exit(); }
$forumId = (int)$reply['forum_id'];
if ($reply['forum_status'] !== 'open') { header("Location: discussion_details.php?id=".$forumId); exit(); }
$replyContent = $reply['content'];

if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_reply'])) {
    $token = $_POST['csrf_token'] ?? '';
    $newContent = trim($_POST['reply_content'] ?? '');
        ...
        try {
            $stmt = $conn->prepare("
                ...
                WHERE dr.reply_id = ? AND dr.user_id = ?
                  ...
                  AND e.enrollment_status = 'Active'
            ");
            $stmt->bind_param("isii", $userId, $newContent, $replyId, $userId);
            $stmt->execute();
            $stmt->close();
            header("Location: discussion_details.php?id=".$forumId."&reply_updated=1");
            exit();
        } catch (Throwable $ex) {
            $errorMessage = 'Unable to update your reply.';
        }
    }
...
<head>
    ...
    <title><?php echo e($projectName); ?> | Edit Reply</title>
    ...
    <div class="content-area">
        ...
        <main class="dashboard-main">
            ...
            <section class="overview-cards">
                <article class="summary-card"><div><p class="card-label">Edit Reply</p><h2><?php echo e($reply['forum_title']); ?></h2></div><div class="summary-icon"><i class="fas fa-reply"></i></div></article>
                <article class="stat-card"><div><p class="card-label">Course</p><h3 class="edit-post-stat"><?php echo e($reply['course_name']); ?></h3></div><span class="stat-badge"><?php echo e($reply['batch_name']); ?></span></article>
            </section>
            <section class="discussion-detail-section">
                <div class="discussion-detail-header"><div><p class="card-label">Your Reply</p><h2>Edit Reply</h2><p>Update your own reply while this forum remains open.</p></div><a href="discussion_details.php?id=<?php echo $forumId; ?>" class="secondary-btn"><i class="fas fa-arrow-left"></i> Cancel</a></div>
                <div class="edit-reply-post-reference"><div class="edit-reply-reference-icon"><i class="fas fa-message"></i></div><div><span>Replying under post</span><strong><?php echo e($reply['post_title']); ?></strong></div></div>
                <form method="POST" action="edit_reply.php?id=<?php echo $replyId; ?>" style="margin-top:20px;">
                    <input type="hidden" name="csrf_token" value="<?php echo e($_SESSION['csrf_token']); ?>">
                    <div class="form-group"><label for="replyContent">Reply</label><textarea class="discussion-textarea" id="replyContent" name="reply_content" required><?php echo e($replyContent); ?></textarea></div>
                    <div class="form-actions"><a href="discussion_details.php?id=<?php echo $forumId; ?>" class="secondary-btn">Cancel</a><button type="submit" name="update_reply" class="primary-btn"><i class="fas fa-floppy-disk"></i> Save Reply</button></div>
                </form>
            </section>
        </main>
    </div>
